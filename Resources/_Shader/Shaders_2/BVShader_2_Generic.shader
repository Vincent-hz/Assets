...
Shader "BVShader/2_Generic" {
    Properties {
        ...
        _SpecularIsMetal ("SpecularIsMetal", Float ) = 1
        _EmissiveColor ("EmissiveColor", Color) = (0,0,0,1)
        _TintColor ("TintColor", Color) = (1,1,1,1)
        _ExtraColor ("ExtraColor", Color) = (0,0,0,1)
    ...
    SubShader {
        ...
        Pass {
            ...
            float3 Combine( float3 diffuse , float3 specular , float3 ambient , float3 emissive , float3 extra ){
            ...
            uniform float _SpecularIsMetal;
            uniform float4 _EmissiveColor;
            float3 LightL( float3 N , float3 L ){
            if(dot(N,L)>0)
               return L;
            else
               return -L;
...
////// Lighting:
                ...
                float3 node_744 = LightL( i.normalDir , lightDirection );
                float4 _DiffuseMap_var = tex2D(_DiffuseMap,TRANSFORM_TEX(i.uv0, _DiffuseMap));
                float3 node_8276 = (lambertianDiffuse( myshadergeom( halfDirection , i.normalDir , node_744 , viewDirection ) )*mixColorWithTint( lerp(_DiffuseColor.rgb,_DiffuseMap_var.rgb,_DiffuseFade) , _TintColor.rgb ));
                float3 node_1054 = heightMapTransform( _BumpMap , i.uv0 , bumpmatrix() , _BumpAmount , i.tangentDir , i.bitangentDir , i.normalDir );
                float4 node_1632 = shaderGeom2( node_1054 , node_744 , viewDirection , halfDirection );
                float node_8523 = customF( node_1632 , _f0 , _f1 , _fresnelPower );
                float node_4878_if_leA = step(_SpecularIsMetal,0.0);
                float node_4878_if_leB = step(0.0,_SpecularIsMetal);
                float3 node_5645 = (wardSpecularIso( node_1054 , halfDirection , node_1632 , _SpecularGloss )*_SpecularColor.rgb);
                float3 finalColor = Combine( diffusecompute( node_8276 , _LightColor0.rgb , node_8523 , attenuation ) , specularcompute( lerp((node_4878_if_leA*node_5645)+(node_4878_if_leB*node_8276),node_5645,node_4878_if_leA*node_4878_if_leB) , _LightColor0.rgb , node_8523 , _SpecularLevel ) , ambientcompute( node_8276 , float3(0.8,0.8,0.8) , node_8523 ) , _EmissiveColor.rgb , _ExtraColor.rgb );
