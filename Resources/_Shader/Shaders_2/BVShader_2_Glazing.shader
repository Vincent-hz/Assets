...
Shader "BVShader/2_Glazing" {
    Properties {
        _DiffuseColor ("DiffuseColor", Color) = (0.5,0.5,0.5,1)
        _DiffuseMap ("DiffuseMap", 2D) = "gray" {}
        ...
        _f0 ("f0", Range(0, 1)) = 0.3872631
        _f1 ("f1", Range(0, 1)) = 0.261972
        _fresnelPower ("fresnelPower", Range(0, 1)) = 0.2050218
        _DiffuseFade ("DiffuseFade", Float ) = 0
        _SpecularLevel ("SpecularLevel", Range(0, 1)) = 0
        _TransparencyLevel ("TransparencyLevel", Range(0, 1)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float3 ambientcompute( float3 diffusemat ){
            return diffusemat;
            }
            ...
            uniform float _TransparencyLevel;
            float Function_node_8158( float transparency ){
            return transparency;
            }
...
////// Lighting:
                ...
                float3 node_8276 = (lambertianDiffuse( myshadergeom( halfDirection , i.normalDir , node_1529 , viewDirection ) )*lerp((node_9188_if_leA*_DiffuseColor.rgb)+(node_9188_if_leB*_DiffuseMap_var.rgb),_DiffuseColor.rgb,node_9188_if_leA*node_9188_if_leB));
                ...
                float3 finalColor = Combine( diffusecompute( node_8276 , _LightColor0.rgb , node_8523 , attenuation ) , specularcompute( (wardSpecularIso( node_1054 , halfDirection , node_1632 , _SpecularGloss )*_SpecularColor.rgb) , _LightColor0.rgb , node_8523 , _SpecularLevel ) , ambientcompute( node_8276 ) );
                fixed4 finalRGBA = fixed4(finalColor,Function_node_8158( _TransparencyLevel ));
...
////// Lighting:
                ...
                float node_8523 = customF( node_1632 , _f0 , _f1 , _fresnelPower );
                float3 finalColor = Combine( diffusecompute( node_8276 , _LightColor0.rgb , node_8523 , attenuation ) , specularcompute( (wardSpecularIso( node_1054 , halfDirection , node_1632 , _SpecularGloss )*_SpecularColor.rgb) , _LightColor0.rgb , node_8523 , _SpecularLevel ) , ambientcompute( node_8276 ) );
                fixed4 finalRGBA = fixed4(finalColor * Function_node_8158( _TransparencyLevel ),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
