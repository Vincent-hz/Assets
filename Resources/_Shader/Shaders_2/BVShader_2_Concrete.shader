...
Shader "BVShader/2_Concrete" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _BrightMap; uniform float4 _BrightMap_ST;
...
////// Lighting:
                ...
                float3 node_8104 = LightL( lightDirection , i.normalDir );
                float4 node_1632 = shaderGeom2( node_1054 , node_8104 , viewDirection , halfDirection );
                ...
                float4 _BrightMap_var = tex2D(_BrightMap,TRANSFORM_TEX(i.uv0, _BrightMap));
                float3 node_8276 = (orenNayarDiffuse( node_1632 , i.normalDir , node_8104 , viewDirection , orenNayarAB( _DiffuseRoughness ) )*(lerp((node_9188_if_leA*_DiffuseColor.rgb)+(node_9188_if_leB*_DiffuseMap_var.rgb),_DiffuseColor.rgb,node_9188_if_leA*node_9188_if_leB)*_BrightMap_var.rgb));
                float node_8523 = customF( node_1632 , _f0 , _f1 , _fresnelPower );
                float3 finalColor = Combine( diffusecompute( node_8276 , _LightColor0.rgb , node_8523 , attenuation ) , specularcompute( (wardSpecularIso( node_1054 , halfDirection , node_1632 , _SpecularGloss )*_SpecularColor.rgb) , _LightColor0.rgb , node_8523 , _SpecularLevel ) , ambientcompute( node_8276 , float3(0.8,0.8,0.8) , node_8523 ) );
