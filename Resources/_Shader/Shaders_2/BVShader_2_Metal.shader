...
Shader "BVShader/2_Metal" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float wardSpecularIso( float4 NL_NV_NH_VH , float2 HTHB , float glossiness , float anisotropy ){
            const float EPSILON = 1e-6f;
                ...
                float2 HTHB2 = HTHB * HTHB;
            
                float NH2 = NL_NV_NH_VH.z * NL_NV_NH_VH.z;
            
                float2 glossTB;
            
                glossTB.x = pow(2.0f, 8.0f * glossiness);
            
                glossTB.y = glossTB.x * anisotropy;
            
                float diff = glossTB.x - SOFTEN_MAX;
            
                if (diff > 0.0f) glossTB.x = SOFTEN_MAX + sqrt(diff);
            
                diff = glossTB.y - SOFTEN_MAX;
            
                if (diff > 0.0f) glossTB.y = SOFTEN_MAX + sqrt(diff);
            
                float2 glossTB2 = glossTB * glossTB;
            
                float dotHTHBGloss = dot(HTHB2, glossTB2);
            
                float result = 0.5f * exp(-dotHTHBGloss / NH2) * glossTB.x * glossTB.y / denom;
            
                glossTB *= 0.5f;
            
                glossTB2 = glossTB * glossTB;
            
                dotHTHBGloss = dot(HTHB2, glossTB2);
            
                result += exp(-dotHTHBGloss / NH2) * glossTB.x * glossTB.y / denom;
                ...
                result += 1.5f * exp(-dotHTHBGloss / NH2) * glossTB.x * glossTB.y / denom;
            ...
            float2 shaderAnisoGeom( float3 H , float3 T , float3 B ){
            float HT = dot(H, T); 
...
////// Lighting:
                ...
                float node_9089 = lambertianDiffuse( node_172 );
                ...
                float3 node_9188 = lerp((node_9188_if_leA*_DiffuseColor.rgb)+(node_9188_if_leB*_DiffuseMap_var.rgb),_DiffuseColor.rgb,node_9188_if_leA*node_9188_if_leB);
                float4 node_1632 = shaderGeom2( heightMapTransform( _BumpMap , i.uv0 , bumpmatrix() , _BumpAmount , i.tangentDir , i.bitangentDir , i.normalDir ) , node_1529 , viewDirection , halfDirection );
                float node_8523 = customF( node_1632 , _f0 , _f1 , _fresnelPower );
                float3 finalColor = Combine( diffusecompute( (lerp((node_1039_if_leA*node_9089)+(node_1039_if_leB*orenNayarDiffuse( node_172 , i.normalDir , node_1529 , viewDirection , orenNayarAB( _DiffuseRoughness ) )),node_9089,node_1039_if_leA*node_1039_if_leB)*node_9188) , _LightColor0.rgb , node_8523 , attenuation ) , specularcompute( (wardSpecularIso( node_1632 , shaderAnisoGeom( halfDirection , i.tangentDir , i.bitangentDir ) , _SpecularGloss , _Anisotropy )*_SpecularColor.rgb) , _LightColor0.rgb , node_8523 , _SpecularLevel ) , ambientcompute( node_9188 , UNITY_LIGHTMODEL_AMBIENT.rgb , node_8523 ) );
