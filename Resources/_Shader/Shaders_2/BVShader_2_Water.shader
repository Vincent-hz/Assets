...
Shader "BVShader/2_Water" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _IOR;
            float customF( float4 NL_NV_NH_VH , float ior ){
            float VHc = 1 - NL_NV_NH_VH.w; 
            
                float VHc2 = VHc *  VHc; 
            
                float f = 4.0f * ior * (VHc * VHc2 * VHc2); 
            
                float2 ior2 = float2(ior - 1, ior + 1 ); 
...
////// Lighting:
                ...
                float3 finalColor = Result( lambertianDiffuse( node_8845 , _DiffuseMap_var.rgb ) , ambient( _DiffuseMap_var.rgb , UNITY_LIGHTMODEL_AMBIENT.rgb ) , specularcompute( (wardSpecularIso( i.normalDir , halfDirection , node_8845 , _SpecularGloss )*_SpecularColor.rgb) , _LightColor0.rgb , customF( node_8845 , _IOR ) , _SpecularLevel ) );
