...
////// Specular:
                ...
                float3 node_2587 = mixColorWithTint( lerp(_DiffuseColor.rgb,_DiffuseMap_var.rgb,_DiffuseFade) , _TintColor.rgb );
                float3 specularColor = node_2587;
                float specularMonochrome;
                float3 diffuseColor = node_2587; // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _DiffuseMap_var = tex2D(_DiffuseMap,TRANSFORM_TEX(i.uv0, _DiffuseMap));
                float3 node_2587 = mixColorWithTint( lerp(_DiffuseColor.rgb,_DiffuseMap_var.rgb,_DiffuseFade) , _TintColor.rgb );
                float3 diffColor = node_2587;
                float3 specColor = node_2587;
