...
////// Specular:
                ...
                float node_3871_if_leA = step(_DiffuseWood,0.0);
                float node_3871_if_leB = step(0.0,_DiffuseWood);
                float4 _DiffuseMap_var = tex2D(_DiffuseMap,TRANSFORM_TEX(i.uv0, _DiffuseMap));
                float3 node_51 = lerp(_DiffuseColor.rgb,_DiffuseMap_var.rgb,_DiffuseFade);
                float3 node_3871 = lerp((node_3871_if_leA*node_51)+(node_3871_if_leB*mixColorWithTint( _DiffuseMap_var.rgb , _DiffuseColor.rgb )),node_51,node_3871_if_leA*node_3871_if_leB);
                float3 specularColor = node_3871;
                float specularMonochrome;
                float3 diffuseColor = node_3871; // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 node_51 = lerp(_DiffuseColor.rgb,_DiffuseMap_var.rgb,_DiffuseFade);
                float3 node_3871 = lerp((node_3871_if_leA*node_51)+(node_3871_if_leB*mixColorWithTint( _DiffuseMap_var.rgb , _DiffuseColor.rgb )),node_51,node_3871_if_leA*node_3871_if_leB);
                float3 diffColor = node_3871;
                float3 specColor = node_3871;
