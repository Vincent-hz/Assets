...
public class FingerEventsSamplePart2 : SampleBase
{
    ...
    #region Utility class that represent a ModelInfo finger path
    ...
    class PathRenderer
    {
        ...
        void UpdateLines()
        {
            ...
            for( int vertexNumbers = 0; vertexNumbers < points.Count; ++vertexNumbers )
                lineRenderer.SetPosition( vertexNumbers, points[vertexNumbers] );
        }
    }
    ...
    protected override void Start()
    {
        ...
        for( int vertexNumbers = 0; vertexNumbers < paths.Length; ++vertexNumbers )
            paths[vertexNumbers] = new PathRenderer( vertexNumbers, lineRendererPrefab );
    }
...
- Blue Circle = FingerUp position";
    ...
    #region GestureMagevent registration/unregistration

    void OnEnable()
    {
        Debug.Log( "Registering finger GestureMagevents from C# script" );
    ...
    void OnDisable()
    {
        // unregister finger GestureMagevents
        FingerGestures.OnFingerDown -= FingerGestures_OnFingerDown;
        FingerGestures.OnFingerUp -= FingerGestures_OnFingerUp;
        FingerGestures.OnFingerMoveBegin -= FingerGestures_OnFingerMoveBegin;
        FingerGestures.OnFingerMove -= FingerGestures_OnFingerMove;
