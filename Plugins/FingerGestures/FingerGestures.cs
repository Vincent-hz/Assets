...
public abstract class FingerGestures : MonoBehaviour
{
    ...
    //下面的事件是默认的手势识别器的包装/代理，这些手势识别器在启动时自动安装和配置，提供了相关的选项
    //只有对每个事件的最相关的数据作为参数传递给事件委托
     //默认的手势识别器可以通过手指操作来访问

    #region Per-Finger events

    //注意:这个类别中的事件是独立于每个手指的，不管其他手指的状态如何。
    //您可以使用手指(指)来检索与手指索引事件参数相对应的手指对象。
    ...
    /// 手势按下事件委托
    /// </summary>
    /// <param name="fingerIndex">基于0的索引唯一识别特定手指的索引</param>
    /// <param name="fingerPos">手指在屏幕上的位置</param>
