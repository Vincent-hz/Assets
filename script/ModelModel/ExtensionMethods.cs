...
public static class ExtensionMethods  {
    ...
    private static void calculateCenter(Transform model, ref Vector3 result, ref int counter)
    {
        ...
        //Transform[] childModels =model.GetComponentsInChildren<Transform>();
        List<Transform> childModels = new List<Transform>();
        for (int i = 0; i < model.GetComponentInChildren<Transform>().GetChildCount(); i++, ++counter)
        {
            childModels.Add(model.GetComponentInChildren<Transform>().GetChild(i));
    ...
    private static void calculateBounds(Transform model, ref Bounds bounds)
    {
        ...
        List<Transform> childModels = new List<Transform>();
        for (int i = 0; i < model.GetComponentInChildren<Transform>().GetChildCount(); i++)
        {
            childModels.Add(model.GetComponentInChildren<Transform>().GetChild(i));
